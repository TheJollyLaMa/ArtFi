...
The bounty workflows only queue payouts when the label matches those exact formats. Queued payroll entries are recorded with `currency: "ART"` in `/home/runner/work/ArtFi/ArtFi/payroll-queue.json`. Payroll bot only recognizes whitelisted contributors.  
